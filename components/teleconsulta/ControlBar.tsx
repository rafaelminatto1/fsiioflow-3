...
const ControlButton: React.FC<{ onClick: () => void; children: React.ReactNode; className?: string; title: string }> = ({ onClick, children, className = '', title }) => (
    <button onClick={onClick} title={title} className={`p-3 rounded-full transition-colors ${className}`}>
        {children}
    </button>
);
...
const ControlBar: React.FC<ControlBarProps> = ({ isMicOn, isCameraOn, onToggleMic, onToggleCamera, onEndCall }) => {
    return (
        <div className="flex justify-center items-center gap-4 bg-slate-800/50 p-2 rounded-xl max-w-sm mx-auto">
            <ControlButton 
                onClick={onToggleMic} 
