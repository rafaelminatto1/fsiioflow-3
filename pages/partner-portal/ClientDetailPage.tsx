...
import { Patient, Voucher, SoapNote, PainLog } from '../../types';
...
const ClientDetailPage: React.FC = () => {
    ...
    const [clientData, setClientData] = useState<{ patient: Patient; voucher: Voucher; soapNotes: SoapNote[]; painLogs: PainLog[] } | null>(null);
    ...
    const { patient, voucher, soapNotes, painLogs } = clientData;
                ...
                <div className="lg:col-span-1 space-y-6">
                    <InfoCard title="Informações do Paciente" icon={<User />}>
                     ...
                     <InfoCard title="Diário de Dor" icon={<LineChart />}>
                        {painLogs.length > 0 ? <PainTrendChart painLogs={painLogs} /> : <p className="text-sm text-slate-500">Nenhum registro de dor.</p>}
                    </InfoCard>
                </div>
                {/* Right Column */}
