...
const QueryLogRow: React.FC<{ log: EconomicAiLog }> = ({ log }) => {
  const source = (log.response as any).provider || (log.response as any).source;
  ...
  const info = (sourceInfo as any)[source as string] || (sourceInfo as any).default;
...
const EconomicPage: React.FC = () => {
  ...
  const usageData = Object.entries(stats.queriesByProvider as Record<string, number>)
    .map(([name, value]) => ({ name, Consultas: value as number }))
    .sort((a, b) => (a.Consultas as number) - (b.Consultas as number));
