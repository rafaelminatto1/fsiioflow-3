...
import * as painLogService from './painLogService';
...
export const getGamificationProgress = async (patientId: string): Promise<GamificationProgress> => {
    ...
    const [appointments, painLogs] = await Promise.all([
        appointmentService.getAppointmentsByPatientId(patientId),
        painLogService.getLogsByPatientId(patientId),
    ...
    totalPoints += painLogs.length * POINTS_CONFIG.PAIN_LOG_ENTRY;
